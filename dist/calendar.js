...
const appointments = [
    new Appointment('Testen', new Date('2021-06-24 15:00:00'), new Date('2021-06-24 16:20:00'), 'red'),
    new Appointment('Testen', new Date('2021-06-25 15:00:00'), new Date('2021-06-25 16:00:00'), 'gray'),
    new Appointment("Essen", new Date('2021-06-27 10:00'), new Date('2021-06-27 10:20'), 'yellow'),
    new Appointment("Essen", new Date('2021-06-27 11:00'), new Date('2021-06-27 11:20'), 'yellow'),
];
export function loadAppointments() {
    const dayElements = [...document.querySelectorAll('.day')];
    const appointmentsByDay = Array(7).map(() => []);
...
function generateGap(duration) {
    const element = document.createElement('p');
    element.innerText = formatTime(duration);
    element.classList.add('gap');
    return element;
}
export function submitNewEventForm(event) {
    event.preventDefault();
    const formData = new FormData(event.target);
    const appointment = new Appointment(String(formData.get('name')), new Date(String(formData.get('start'))), new Date(String(formData.get('end'))), 'yellow');
...
function generateAppointmentHTML(appointment) {
    const template = document.querySelector('#appointment-template');
